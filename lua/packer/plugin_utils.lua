...
plugin_utils.helptags_stale = function(dir)
    ...
    local txt_newest = math.max(table.unpack(txt_ftimes))
    local tag_oldest = math.min(table.unpack(tag_ftimes))
            ...
            for _, task in ipairs(plugin.run) do
                ...
                elseif type(task) == 'string' then
                    if string.sub(task, 1, 1) == ':' then
                        ...
                        hook_result = await(jobs.run(cmd, { capture_output = hook_callbacks, cwd = plugin.install_path }))
                            :map_err(
                                function(err)
                                    return {
