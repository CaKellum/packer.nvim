    ...
    return async(function()
        ...
        if plugin.tag and has_wildcard(plugin.tag) then
            ...
            if data then
                ...
                log.warn(
                    fmt('Wildcard expansion did not found any tag for plugin %s: defaulting to latest commit...',
                        plugin.name)
                )
        ...
        if plugin.branch or (plugin.tag and not opts.preview_updates) then
            ...
            if disp ~= nil then
                disp:task_update(plugin_name,
                    fmt('checking out %s %s...', plugin.branch and 'branch' or 'tag', branch_or_tag))
            end
...
local get_rev = function(plugin)
    ...
    return async(function()
        local rev = await(jobs.run(rev_cmd,
...
git.setup = function(plugin)
    ...
    plugin.installer = function(disp)
        ...
        return async(function()
            ...
            if plugin.commit then
                disp:task_update(plugin_name, fmt('checking out %s...', plugin.commit))
                r:and_then(await,
                    jobs.run(config.exec_cmd .. fmt(config.subcommands.checkout, plugin.commit), installer_opts))
                    :map_err(function(err)
                        return {
                            msg = fmt('Error checking out commit %s for %s', plugin.commit, plugin_name),
                            data = { err, output },
    ...
    plugin.diff = function(commit, callback)
        async(function()
            ...
            return await(jobs.run(diff_cmd,
                    { capture_output = diff_callbacks, cwd = install_to, options = { env = git.job_env } }))
                :map_ok(function(_)
