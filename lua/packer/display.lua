...
local display_mt = {
            ...
            for key, value in pairs(plug_conf) do
                if vim.tbl_contains(status_keys, key) then
                    ...
                    if type(details) == 'string' then
                        ...
                        vim.list_extend(config_lines, { fmt('%s%s: ', padding, key), table.unpack(details) })
                    end
    ...
    continue = function(self)
        ...
        if #plugins > 0 then
            require('packer').update({ pull_head = true, preview_updates = false }, table.unpack(plugins))
        else
...
display.retry = function()
    ...
    elseif #display.status.failed_update_list > 0 then
        require('packer').update(table.unpack(display.status.failed_update_list))
    end
end
